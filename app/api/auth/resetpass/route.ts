...
export async function POST(req: Request) {
  ...
  if (baseData === null) {
    return NextResponse.json(
      { message: "Database error", resetState: false },
      { status: 500 }
    );
  }
  ...
  if (baseData.length === 0) {
    return NextResponse.json(
      { message: "No such user", resetState: false },
      { status: 200 }
    );
  } else {
    ...
    if (!(await writeResetTokenToBase(email, resettoken, expireTime))) {
      return NextResponse.json(
        { message: "Database error for saving reset token", resetState: false },
        { status: 500 }
      );
    } else {
      const mailOptions = {
